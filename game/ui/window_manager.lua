...
function WindowManager:draw()
    ...
    for _, object in pairs(self.objects) do
        if object.position == "relative" then
            local xPos =
                (object.right and ((self.x + self.width) - object.right) or
                (object.left and (self.x + object.left) or self.x + object.x))
            local yPos =
                (object.down and ((self.y + self.height) - object.down) or
                (object.up and (self.y + object.up) or self.y + object.y))
            object:drawObject(xPos, yPos, object.angle, object.width, object.height)
